...
| HERMES_ENV_RESOLVE_PATCH_V1 | fix: resolve Paperclip structured env before hermes adapter dispatch | server/src/adapters/registry.ts | `hermes-paperclip-adapter@0.3.0` execute.js reads `ctx.agent.adapterConfig.env` (raw DB record with `{type:"plain",value:"..."}` objects) instead of `ctx.config.env` (resolved plain strings). Wrapper `hermesExecuteWithEnvResolution()` merges resolved env from `ctx.config.env` into `agent.adapterConfig` before calling hermesExecute. Without this, `HERMES_HOME` arrives as `[object Object]` → Hermes falls back to global `~/.hermes` config → resolves Anthropic provider → auth error | HIGH | Bug report filed upstream |
